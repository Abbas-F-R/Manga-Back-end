...
public class GenericRepository<T, TId> : IGenericRepository<T, TId> where T : BaseEntity
{
    ...
    public async Task<(List<T>? data, int? totalCount)> GetAll(Expression<Func<T, bool>>? predicate,
        ...
        bool? orderByViews = false, bool? orderByUpdatingDate = false,
        bool deleted = false)
    {
        ...
        if (orderByViews == true)
        {
            query = query.OrderByDescending(model => EF.Property<long>(model, "Views")); // Adjust if Views is not an int
        }
        else if (orderByUpdatingDate == true)
        {
            query = query.OrderByDescending(model => EF.Property<DateTime>(model, "UpdatingDate")); // Adjust if UpdatingDate is not a DateTime
        }
        else
        {
